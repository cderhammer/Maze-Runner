...
public class Mouse extends AbstractMouse {
    ...
    public void move(List<Location> emptyLocations) {
        if (emptyLocations.size() != 0) {
            this.crumbs.push(this.getLocation());
            Random loc = new Random();
            int randomLoc = loc.nextInt(emptyLocations.size());
            this.moveTo(emptyLocations.get(randomLoc));
        } else {
            try {
    ...
    // Previous code that can be useful elsewhere

    /*  try {
            boolean empty = false;
            ArrayListStack newList = new ArrayListStack();
            Location loc = crumbs.peek();
            if (loc == emptyLocations) {
                empty = true;
